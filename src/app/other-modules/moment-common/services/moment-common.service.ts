...
export class MomentCommonService {
  ...
  getNowFormated(): string {
    return this.getNow().format(
      `${DEFAULT_APP_VALUES.dateLocalFormat}THH:mm:ss`
    );
  }
  ...
  getFormatedDate(date?: moment.Moment): string {
    return date
      ? date.format(DEFAULT_APP_VALUES.dateLocalFormat)
      : this.getToday().format(DEFAULT_APP_VALUES.dateLocalFormat);
  }
  ...
  isCurrencyNbpValidDate(date: moment.Moment): moment.Moment {
    if (date instanceof moment) {
      ...
      if (date.day() == 0 || date.day() == 6) {
        date =
          date.day() == 0 ? date.subtract(2, "day") : date.subtract(1, "day");
      }
    }
    return date;
  }
