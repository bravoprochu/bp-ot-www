...
import { ILoadTransEu, ITransEuPallete } from 'app/shared/interfaces/i-load-trans-eu';
...
export class LoadTransEuComponent implements OnInit, OnDestroy {
  ...
  transEuLoadMapper(rform:ILoad)
  {
      ...
      this.loadTransEu=<ILoadTransEu>{
        ...
        is_tir_cable_required: rform.buy.load_info.extraInfo.is_tir_cable_required,
        is_tracking_system_required: rform.buy.load_info.extraInfo.is_tracking_system_required,
        is_truck_crane_required: rform.buy.load_info.extraInfo.is_truck_crane_required,
        ...
        type_of_load: {
          id: rform.buy.load_info.extraInfo.type_of_load !=null? rform.buy.load_info.extraInfo.type_of_load.value: null
        },
        contact_person: [],
     ...
     //ADR
     rform.buy.load_info.extraInfo.required_adr_classes.forEach((a:IValueViewValue)=>{
       this.loadTransEu.required_adr_classes.push(a.value);
     });
     rform.buy.load_info.extraInfo.required_ways_of_loading.forEach((load:IValueViewValue)=>{
  ...
  private prepPallets(frm:ILoad)
  {
    ...
    if(pallets.length>1){
      ...
      pallets.forEach(pallet => {
        if(idx>0){
          let dimmension = pallet.type.value=="Other" ? pallet.dimmension: "";
          ...
          let palletInfo=pallet.info!=null && pallet.info.length>0? `(${pallet.info})`: "";
