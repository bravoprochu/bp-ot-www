...
export class InvoiceSellComponent implements OnInit, OnDestroy, IDetailObj {
  ...
  prepTotalInWord(): string {
    ...
    return `${this.cf.inWords(totalBrutto)} ${rest}/100 ${
  ...
  prepExtraInfoTaxExchangedNbp(_currNbp: ICurrencyNbp): void {
    ...
    let info = `Średni kurs dla ${
      ...
      _currNbp.rate
    }) ||| Netto (${netto}) wartość: ${nettoPLN} PLN ||| Podatek VAT (${this.cf.roundToCurrency(
      _currNbp.price
    )}) wartość: ${this.cf.roundToCurrency(
      _currNbp.plnValue
